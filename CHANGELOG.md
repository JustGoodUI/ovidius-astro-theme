...
- simplified canonical and share URLs to use `Astro.url.pathname` with the configured `site`
- removed `astro check` from the build step, still available via the `check` script
...
- fixed footer spacing between the year and site link (Astro v7 whitespace handling)
